  ...
  return (
    <ModalFrame title={editing ? 'Настроить группу' : 'Новая группа'} onClose={onClose} closeDisabled={saving}>
      <div className="mt-6 space-y-4">
        <label className="block"><span className="mb-1.5 block text-xs font-bold text-slate-600">Курс *</span><select value={courseId} disabled={editing || saving} onChange={event => setCourseId(event.target.value)} className="min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold outline-none focus:border-[#1B4FD8] focus:ring-2 focus:ring-[#1B4FD8]/15 disabled:bg-slate-50"><option value="">Выберите активный курс</option>{courses.filter(course => course.isActive || course.id === group?.course.id).map(course => <option key={course.id} value={course.id}>{course.name} · {courseMeta(course)}</option>)}</select>{editing && <p className="mt-1.5 text-xs leading-5 text-slate-400">Курс нельзя менять после создания: это сохраняет учебный контекст назначений.</p>}</label>
        <label className="block"><span className="mb-1.5 block text-xs font-bold text-slate-600">Название *</span><input value={name} onChange={event => setName(event.target.value)} maxLength={160} placeholder="Например, ОРТ-11 / вечер" className="min-h-11 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-[#1B4FD8] focus:ring-2 focus:ring-[#1B4FD8]/15" /></label>
        <div className="grid gap-4 sm:grid-cols-2"><label className="block"><span className="mb-1.5 block text-xs font-bold text-slate-600">Формат</span><select value={deliveryMode} onChange={event => setDeliveryMode(event.target.value as AdminGroupDeliveryMode)} disabled={saving} className="min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-[#1B4FD8] focus:ring-2 focus:ring-[#1B4FD8]/15">{(Object.keys(MODE_LABELS) as AdminGroupDeliveryMode[]).map(mode => <option key={mode} value={mode}>{MODE_LABELS[mode]}</option>)}</select></label><label className="block"><span className="mb-1.5 block text-xs font-bold text-slate-600">Вместимость</span><input type="number" value={capacity} onChange={event => setCapacity(event.target.value)} min={1} max={5_000} placeholder="Без лимита" disabled={saving} className="min-h-11 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-[#1B4FD8] focus:ring-2 focus:ring-[#1B4FD8]/15" /></label></div>
        <div className="grid gap-4 sm:grid-cols-2"><label className="block"><span className="mb-1.5 block text-xs font-bold text-slate-600">Начало</span><input type="date" value={startsOn} onChange={event => setStartsOn(event.target.value)} disabled={saving} className="min-h-11 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-[#1B4FD8] focus:ring-2 focus:ring-[#1B4FD8]/15" /></label><label className="block"><span className="mb-1.5 block text-xs font-bold text-slate-600">Окончание</span><input type="date" value={endsOn} onChange={event => setEndsOn(event.target.value)} disabled={saving} className="min-h-11 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-[#1B4FD8] focus:ring-2 focus:ring-[#1B4FD8]/15" /></label></div>
...
function TeacherAssignmentModal({ group, teachers, onClose, onSaved }: {
  ...
  return (
    <ModalFrame title="Преподаватель группы" onClose={onClose} closeDisabled={saving}>
      ...
      <label className="mt-5 block"><span className="mb-1.5 block text-xs font-bold text-slate-600">Преподаватель</span><select value={teacherId} onChange={event => setTeacherId(event.target.value)} disabled={saving} className="min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-[#1B4FD8] focus:ring-2 focus:ring-[#1B4FD8]/15"><option value="">Не назначен</option>{teachers.map(teacher => <option key={teacher.id} value={teacher.id}>{teacher.fullName} · {teacher.email}</option>)}</select></label>
      ...
      <div className="mt-6 flex flex-wrap gap-3"><button type="button" onClick={() => void submit()} disabled={saving} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#1B4FD8] px-5 text-sm font-bold text-white hover:bg-blue-700 disabled:opacity-60"><GraduationCap size={17} aria-hidden="true" />{saving ? 'Сохраняем…' : 'Сохранить'}</button><button type="button" onClick={onClose} disabled={saving} className="min-h-11 rounded-xl bg-slate-100 px-5 text-sm font-bold text-slate-600 hover:bg-slate-200">Отмена</button></div>
    </ModalFrame>
  )
}
...
function StudentAssignmentModal({ group, students, existingIds, onClose, onAdded }: {
  ...
  return (
    <ModalFrame title="Добавить ученика" onClose={onClose} closeDisabled={saving}>
      ...
      <label className="mt-5 block"><span className="mb-1.5 block text-xs font-bold text-slate-600">Ученик</span><select value={studentId} onChange={event => setStudentId(event.target.value)} disabled={saving || eligible.length === 0} className="min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-[#1B4FD8] focus:ring-2 focus:ring-[#1B4FD8]/15"><option value="">Выберите ученика</option>{eligible.map(student => <option key={student.id} value={student.id}>{student.fullName} · {student.email} · {studentTypeLabel(student.studentType!)}</option>)}</select></label>
      ...
      {error && <p role="alert" className="mt-4 rounded-xl border border-red-200 bg-red-50 px-3 py-2.5 text-sm font-semibold text-red-700">{error}</p>}
      <div className="mt-6 flex flex-wrap gap-3"><button type="button" onClick={() => void submit()} disabled={saving || eligible.length === 0} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#1B4FD8] px-5 text-sm font-bold text-white hover:bg-blue-700 disabled:opacity-60"><UserPlus size={17} aria-hidden="true" />{saving ? 'Добавляем…' : 'Добавить'}</button><button type="button" onClick={onClose} disabled={saving} className="min-h-11 rounded-xl bg-slate-100 px-5 text-sm font-bold text-slate-600 hover:bg-slate-200">Отмена</button></div>
    </ModalFrame>
  )
}
  ...
  return (
    <div className="min-h-screen bg-[#FAF8FF]">
      ...
      <main className="mx-auto max-w-7xl space-y-5 px-4 py-6 sm:px-6">
        <section className="rounded-2xl border border-blue-100 bg-blue-50/80 p-4 sm:flex sm:items-center sm:justify-between sm:gap-5"><div className="flex gap-3"><span className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-[#1B4FD8] shadow-sm"><Users size={20} aria-hidden="true" /></span><div><h1 className="text-sm font-black text-[#0D1E4A]">Группы работают через собственный backend Zhangak</h1><p className="mt-1 max-w-3xl text-sm leading-5 text-slate-600">Создайте группу внутри курса, назначьте преподавателя и добавьте учеников. История исключения сохраняется, а формат обучения проверяется на сервере.</p></div></div><button type="button" onClick={() => setFormTarget('create')} className="mt-3 inline-flex min-h-10 shrink-0 items-center gap-2 rounded-xl bg-[#1B4FD8] px-4 text-sm font-bold text-white hover:bg-blue-700 sm:mt-0"><Plus size={16} aria-hidden="true" />Новая группа</button></section>
