...
import { listAdminCoursePracticeTests, listAllAdminPracticeQuestions, type AdminPracticeQuestion, type AdminPracticeTest } from '@/lib/admin-assessments-client'
...
export default function AdminDailyChallengePage() {
  ...
  useEffect(() => {
    let active = true
    const timer = window.setTimeout(() => {
      if (testId === null) { setQuestions([]); setQuestionsLoading(false); return }
      void (async () => {
        setQuestionsLoading(true)
        setError('')
        try {
          const activeQuestions = await listAllAdminPracticeQuestions(testId, { status: 'active' })
          if (active) setQuestions(activeQuestions)
        } catch (cause) {
          if (active) setError(message(cause, 'Не удалось загрузить вопросы'))
        } finally {
