...
export default function AdminDailyChallengePage() {
  ...
  const load = async () => {
    const [list, students, settingsRes] = await Promise.all([
      ...
      authenticatedFetch('/api/admin/settings').then(r => r.json()).catch(() => ({ settings: {} })),
    ])
    setChallenges(list)
  ...
  const handleGenerateNow = async () => {
    ...
    try {
      ...
      const createRes = await authenticatedFetch('/api/admin/daily-challenge', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const genRes = await authenticatedFetch('/api/admin/daily-challenge/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleCreate = async () => {
    ...
    setError(null)
    try {
      const res = await authenticatedFetch('/api/admin/daily-challenge', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
