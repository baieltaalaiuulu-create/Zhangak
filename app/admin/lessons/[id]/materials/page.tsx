...
import { AlertTriangle, ArrowLeft, CheckCircle2, Eye, EyeOff, FileText, LoaderCircle, Upload, Video, Wrench, XCircle } from 'lucide-react'
...
export default function AdminLessonMaterialsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // replacement is re-normalized server-side, so an invalid one is rejected.
  const repair = async (material: AdminLessonMaterial) => {
    if (saving || !lessonId) return
