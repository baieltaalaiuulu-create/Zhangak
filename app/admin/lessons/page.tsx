      ...
      <main className="mx-auto max-w-7xl space-y-5 px-4 py-6 sm:px-6">
        <section className="rounded-2xl border border-blue-100 bg-blue-50/70 p-4 sm:flex sm:items-center sm:justify-between sm:gap-5">
          ...
          <button type="button" onClick={() => setCourseCreateOpen(true)}
            className="mt-3 inline-flex min-h-10 shrink-0 items-center gap-1.5 rounded-xl border border-[#1B3F92]/20 bg-white px-4 text-sm font-bold text-[#1B3F92] hover:bg-blue-50 sm:mt-0">
        ...
        <section aria-labelledby="courses-heading">
          ...
          {coursesLoading ? (
            ...
            <div className="rounded-2xl border border-dashed border-gray-300 bg-white px-5 py-9 text-center">
              <BookOpen size={24} className="mx-auto text-[#1B3F92]" aria-hidden="true" />
              ...
              <button type="button" onClick={() => setCourseCreateOpen(true)} className="mt-4 rounded-xl bg-[#1B3F92] px-4 py-2.5 text-sm font-bold text-white hover:bg-blue-700">Создать первый курс</button>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {courses.map(course => {
                ...
                return (
                  <button key={course.id} type="button" onClick={() => setSelectedCourseId(course.id)}
                    className={`min-h-32 rounded-2xl border p-4 text-left transition-colors ${selected ? 'border-[#1B3F92] bg-[#EEF2FF] shadow-sm' : 'border-gray-200 bg-white hover:border-[#1B3F92]/40 hover:bg-blue-50/30'}`}>
                    <div className="flex items-start justify-between gap-3">
                      <span className={`flex h-9 w-9 items-center justify-center rounded-xl ${selected ? 'bg-[#1B3F92] text-white' : 'bg-blue-50 text-[#1B3F92]'}`}><BookOpen size={18} aria-hidden="true" /></span>
                    ...
                    <p className="mt-3 text-xs font-semibold text-[#1B3F92]">{course.lessonCount} {course.lessonCount === 1 ? 'урок' : course.lessonCount >= 2 && course.lessonCount <= 4 ? 'урока' : 'уроков'}</p>
                  </button>
                )
              })}
            </div>
        ...
        <section aria-labelledby="lessons-heading" className="rounded-2xl border border-gray-200 bg-white">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 px-4 py-4 sm:px-5">
            ...
            {selectedCourse && (
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button type="button" onClick={openNewLesson} className="inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-[#1B3F92] px-4 text-sm font-bold text-white hover:bg-blue-700"><Plus size={16} /> Добавить урок</button>
              </div>
            )}
          </div>
          ...
          {selectedCourseId === null ? (
            ...
            <div className="px-5 py-10 text-center">
              <BookOpen size={22} className="mx-auto text-[#1B3F92]" aria-hidden="true" />
              <p className="mt-3 text-sm font-bold text-[#191B23]">В этом курсе пока нет уроков</p>
              <button type="button" onClick={openNewLesson} className="mt-3 text-sm font-bold text-[#1B3F92] hover:text-blue-700">Добавить первый урок</button>
            </div>
          ) : (
                ...
                <tbody>
                  {lessons.map(lesson => (
                    <tr key={lesson.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                      ...
                      <td className="px-3 py-3">
                        {lesson.contentUrl ? <a href={lesson.contentUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs font-bold text-[#1B3F92] hover:text-blue-700"><ExternalLink size={13} /> Открыть</a> : <span className="text-xs text-gray-400">Не добавлен</span>}
                      </td>
                      <td className="px-3 py-3">
                      ...
                      <td className="px-3 py-3">
                        <div className="flex items-center justify-end gap-1">
                          <button type="button" onClick={() => router.push(`/admin/lessons/${lesson.id}/questions`)} aria-label={`Статус вопросов урока ${lesson.title}`} className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 hover:text-[#1B3F92]"><ListChecks size={15} /></button>
                          <button type="button" onClick={() => setEditTarget(lesson)} aria-label={`Редактировать урок ${lesson.title}`} className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 hover:text-[#1B3F92]"><Pencil size={15} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
