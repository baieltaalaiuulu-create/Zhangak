...
import { CalendarDays, CheckCircle2, Flame, RefreshCw, Sparkles, Trophy } from 'lucide-react'

import { useStudentSession } from '@/components/student/StudentSessionContext'
import { getGamificationSummary, type GamificationSummary, type QuestProgress } from '@/lib/platform-community'
...
function QuestCard({ quest }: { quest: QuestProgress }) {
  ...
  return (
    <article className={`rounded-2xl border p-4 transition-transform duration-300 motion-reduce:transition-none ${complete ? 'border-emerald-200 bg-emerald-50/70' : 'border-[#E4E1F8] bg-white hover:-translate-y-0.5 motion-reduce:hover:translate-y-0'}`}>
      <div className="flex items-start gap-3">
        <span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl ${complete ? 'bg-emerald-500 text-white' : 'bg-violet-100 text-violet-700'}`}>
          {complete ? <CheckCircle2 size={23} aria-hidden="true" /> : <Sparkles size={22} aria-hidden="true" />}
        </span>
      ...
      <div className="mt-4 flex items-center justify-between gap-3 text-xs font-bold"><span className={complete ? 'text-emerald-700' : 'text-slate-600'}>{complete ? 'Квест выполнен' : `${quest.currentCount} из ${quest.targetCount}`}</span><span className="text-slate-400">{complete ? 'Награда начислена' : `Осталось ${timeLeft(quest.periodEnd)}`}</span></div>
  ...
  return (
    <main className="mx-auto min-h-screen max-w-2xl px-4 pb-28 pt-5 sm:px-6 md:pb-10">
      <header className="rounded-[26px] bg-gradient-to-br from-[#1B3F92] to-[#6C3DE0] p-5 text-white shadow-[0_10px_24px_rgba(40,45,120,0.2)]">
        <div className="flex items-start gap-3"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white/15"><Trophy size={26} aria-hidden="true" /></span><div className="min-w-0"><p className="text-[11px] font-extrabold uppercase tracking-[.12em] text-white/70">Твой игровой прогресс</p><h1 className="mt-1 text-2xl font-black">Квесты Zhangak</h1><p className="mt-1 text-sm text-white/80">Выполняй действия в учёбе — награды начисляются автоматически.</p></div></div>
