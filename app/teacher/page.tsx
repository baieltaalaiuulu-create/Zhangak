    ...
    <div className="flex min-h-screen items-center justify-center bg-[#F6F7FB] p-5">
      <div className="w-full max-w-md rounded-3xl border border-red-100 bg-white p-7 text-center shadow-sm">
        ...
        <div className="mt-5 flex flex-wrap justify-center gap-3">
          {signInRequired || wrongRole ? <button type="button" onClick={() => window.location.assign('/login?surface=platform')} className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-[#1B4FD8] px-5 text-sm font-bold text-white"><LogIn size={17} aria-hidden="true" />Войти</button> : <button type="button" onClick={() => void onRetry()} className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-[#1B4FD8] px-5 text-sm font-bold text-white"><RefreshCw size={17} aria-hidden="true" />Повторить</button>}
        </div>
      </div>
    </div>
  )
