...
function StatCard({ value, suffix = '', label, color, delay }: {
  ...
  return (
    <div ref={ref} className="stat-card" style={{
      ...
      <div style={{ color: 'rgba(255,255,255,0.55)', fontSize: '12px', marginTop: '6px', fontWeight: '500' }}>{label}</div>
    </div>
  )
}
...
export default function LandingPage() {
  ...
  const courses = [
    ...
    {
      level: 'C1', name: 'Финальный', month: '3-ай', price: '5 500',
    ...
    <div style={{ background: '#050C1F', minHeight: '100vh', fontFamily: 'Inter, -apple-system, sans-serif', color: '#fff', overflowX: 'hidden' }}>
      ...
      <div style={{
        ...
        <a href={wa} target="_blank" rel="noopener noreferrer"
          style={{ color: '#fff', marginLeft: '14px', textDecoration: 'none', background: 'rgba(255,255,255,0.2)', padding: '3px 12px', borderRadius: '20px', fontWeight: '800' }}>
          Жазылуу →
        </a>
      </div>
      ...
      <nav className="section-pad" style={{
        ...
        <div className="nav-links" style={{ display: 'flex', gap: '28px' }}>
          {[['Курстар', '#courses'], ['Натыйжалар', '#results'], ['FAQ', '#faq'], ['Офис', '#office']].map(([l, h]) => (
            <a key={l} href={h} className="nav-link" style={{ color: 'rgba(255,255,255,0.5)', fontSize: '14px', textDecoration: 'none', fontWeight: '500', transition: 'color 0.2s' }}>{l}</a>
          ))}
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
          <button onClick={() => setShowLogin(true)}
            style={{ background: 'rgba(255,255,255,0.07)', color: '#fff', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '10px', padding: '9px 18px', fontWeight: '600', fontSize: '13px', cursor: 'pointer', transition: 'all 0.2s' }}>
            Кирүү
          </button>
        ...
        <div className="section-pad hero-grid" style={{ padding: '80px 60px', maxWidth: '1200px', margin: '0 auto', width: '100%', display: 'flex', alignItems: 'center', gap: '60px', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ animation: 'slideInLeft 0.8s 0.2s cubic-bezier(.22,1,.36,1) both' }}>
              <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: '16px', lineHeight: '1.8', marginBottom: '36px', maxWidth: '460px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', animation: 'slideInLeft 0.8s 0.3s cubic-bezier(.22,1,.36,1) both' }}>
              ...
              <button onClick={() => setShowLogin(true)} className="cta-btn"
                style={{ background: 'rgba(255,255,255,0.07)', color: '#fff', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '14px', padding: '15px 32px', fontWeight: '700', fontSize: '15px', cursor: 'pointer', transition: 'all 0.25s' }}>
                Кирүү →
              </button>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              {results.map((r, i) => (
                <div key={i} style={{
                  ...
                  aspectRatio: '3/4', border: '1px solid rgba(255,255,255,0.08)',
      ...
      <div className="section-pad" style={{ padding: '0 60px 60px' }}>
        <Reveal>
          <div style={{
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
                {[['📅', '1-июлдан'], ['🕙', '10:00–12:50'], ['📍', 'Горький 108'], ['💰', '7 000 сом']].map(([icon, text]) => (
                  <div key={text} style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', color: 'rgba(255,255,255,0.75)', background: 'rgba(255,255,255,0.06)', borderRadius: '10px', padding: '6px 12px' }}>
            ...
            <a href={wa} target="_blank" rel="noopener noreferrer" className="cta-btn"
              style={{ background: 'linear-gradient(135deg,#F59E0B,#EF4444)', color: '#fff', borderRadius: '16px', padding: '18px 36px', fontWeight: '900', fontSize: '16px', textDecoration: 'none', textAlign: 'center', flexShrink: 0, boxShadow: '0 8px 32px rgba(245,158,11,0.4)', transition: 'all 0.25s', whiteSpace: 'nowrap' }}>
              📲 Азыр жазылуу<br />
      ...
      {/* ── TIMER ─────────────────────────────────────────────────────────────── */}
      <div className="section-pad" style={{ padding: '60px 60px', background: 'rgba(255,255,255,0.02)', borderTop: '1px solid rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        <Reveal>
          <div style={{ textAlign: 'center', maxWidth: '700px', margin: '0 auto' }}>
            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '12px', fontWeight: '700', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '20px' }}>ЖРТга чейин калды</p>
            <div className="timer-grid" style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap', alignItems: 'center' }}>
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  {i < 3 && <span style={{ fontSize: '28px', fontWeight: '900', color: 'rgba(255,255,255,0.2)' }}>:</span>}
                </div>
              ))}
            </div>
            <a href={wa} target="_blank" rel="noopener noreferrer" className="cta-btn"
            ...
            {courses.map((c, i) => (
              <Reveal key={c.level} delay={i * 120}>
                <div className="course-card" style={{
                  ...
                  {c.featured && (
                    <div style={{ position: 'absolute', top: '16px', right: '16px', background: c.color, color: '#fff', fontSize: '10px', fontWeight: '900', padding: '4px 12px', borderRadius: '20px', letterSpacing: '0.5px' }}>
                      ⭐ {c.badge}
                    </div>
                  )}
                  {/* Color bar */}
                  ...
                  <div style={{ padding: '28px' }}>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
                      ...
                      <span style={{ color: 'rgba(255,255,255,0.35)', fontSize: '12px' }}>{c.month}</span>
                    </div>
                    ...
                    <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', paddingTop: '22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div>
                        <div style={{ fontWeight: '900', fontSize: '22px', color: c.color }}>{c.price}</div>
                        <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.35)' }}>сом / ай</div>
                      </div>
                      <a href={wa} target="_blank" rel="noopener noreferrer" className="cta-btn"
                        style={{ background: c.color, color: '#fff', borderRadius: '12px', padding: '11px 22px', fontSize: '13px', fontWeight: '800', textDecoration: 'none', transition: 'all 0.25s', boxShadow: `0 4px 20px ${c.glow}` }}>
                        Жазылуу
                      </a>
                    </div>
                  </div>
      ...
      {/* ── RESULTS ───────────────────────────────────────────────────────────── */}
      <div id="results" className="section-pad" style={{ padding: '100px 60px', background: 'rgba(255,255,255,0.015)' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <Reveal>
            <div style={{ textAlign: 'center', marginBottom: '64px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '16px' }}>
            {results.map((r, i) => (
              <Reveal key={i} delay={i * 100}>
                <div style={{ position: 'relative', borderRadius: '20px', overflow: 'hidden', aspectRatio: '3/4', border: '1px solid rgba(255,255,255,0.07)', cursor: 'pointer', transition: 'transform 0.3s ease' }}
            ...
            {reviews.map((r, i) => (
              <Reveal key={i} delay={i * 80}>
                <div className="review-card" style={{
                  ...
                  <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '13px', lineHeight: '1.75', fontStyle: 'italic', margin: 0 }}>"{r.text}"</p>
      ...
      {/* ── OFFICE ────────────────────────────────────────────────────────────── */}
      <div id="office" className="section-pad" style={{ padding: '100px 60px', background: 'rgba(255,255,255,0.015)' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <Reveal>
          ...
          <Reveal delay={100}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: '20px' }}>
              <div style={{ borderRadius: '20px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.08)', minHeight: '300px' }}>
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '20px', padding: '28px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <div>
                  <div style={{ fontWeight: '900', fontSize: '17px', marginBottom: '4px' }}>Жангак офиси</div>
                  <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '13px' }}>Бишкек, Кыргызстан</div>
                </div>
              ...
              <Reveal key={i} delay={i * 60}>
                <div style={{
                  ...
                  <button onClick={() => setOpenFaq(openFaq === i ? null : i)}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px', background: 'none', border: 'none', color: '#fff', cursor: 'pointer', textAlign: 'left', gap: '16px' }}>
                  ...
                  {openFaq === i && (
                    <div style={{ padding: '0 24px 20px', color: 'rgba(255,255,255,0.6)', fontSize: '14px', lineHeight: '1.75', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <div className="section-pad" style={{ padding: '100px 60px', position: 'relative', overflow: 'hidden' }}>
        ...
        <Reveal>
          <div style={{ maxWidth: '600px', margin: '0 auto', textAlign: 'center', position: 'relative', zIndex: 1 }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.5)', marginBottom: '36px', fontSize: '16px', lineHeight: '1.7' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
              ...
              <button onClick={() => setShowLogin(true)} className="cta-btn"
                style={{ background: 'rgba(255,255,255,0.07)', color: '#fff', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '16px', padding: '17px 40px', fontWeight: '700', fontSize: '16px', cursor: 'pointer', transition: 'all 0.25s' }}>
                Кирүү →
              </button>
            </div>
          </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(16px)', padding: '20px' }}
          ...
          <div style={{ background: '#0D1425', border: '1px solid rgba(27,79,216,0.3)', borderRadius: '28px', padding: '44px', width: '100%', maxWidth: '420px', position: 'relative', boxShadow: '0 24px 80px rgba(27,79,216,0.3)', animation: 'bounceIn 0.4s cubic-bezier(.22,1,.36,1) both' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: '32px' }}>
              ...
              <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '13px', marginTop: '6px' }}>Жангак системасына кирүү</div>
            </div>
            <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {[
                ...
                <div key={f.label}>
                  <label style={{ fontSize: '12px', fontWeight: '600', color: 'rgba(255,255,255,0.45)', display: 'block', marginBottom: '7px' }}>{f.label}</label>
                  <input type={f.type} value={f.val} onChange={e => f.setter(e.target.value)} placeholder={f.placeholder} required
                    style={{ width: '100%', padding: '13px 16px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)', fontSize: '15px', outline: 'none', color: '#fff', background: 'rgba(255,255,255,0.05)', boxSizing: 'border-box', transition: 'border-color 0.2s' }}
