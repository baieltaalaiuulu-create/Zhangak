...
export default function LandingPage() {
  ...
  const reviews = [
    { name: 'Айбек С.', score: 180, before: 110, text: 'Курска чейин математиканы такыр түшүнбөй, 110 балл алчумун. Жангакта 3 ай окугандан кийин 180 балл алдым. Мугалимдер абдан жакшы түшүндүрөт, ар бир тема боюнча практика көп.', emoji: '🎯' },
    { name: 'Нурайым К.', score: 195, before: 140, text: 'Аналогияларды эч качан түшүнбөй келдим. Жангакта конкреттүү стратегияларды үйрөндүм. Натыйжада 195 балл алдым жана каалаган университетиме кирдим!', emoji: '⭐' },
    { name: 'Тимур А.', score: 188, before: 125, text: '3 сааттык сабак чарчатпайт, анткени аралыкта оюн убактысы бар. Мугалимдер студенттерге жекече мамиле кылат. Менин баллым 125тен 188ге чыкты.', emoji: '🚀' },
    { name: 'Зарина М.', score: 204, before: 155, text: 'Математика боюнча геометрия темасы мен үчүн абдан кыйын болчу. Жангактын мугалими ар бир темага убакыт коротуп, толук түшүндүрдү. 204 балл алдым!', emoji: '💪' },
    { name: 'Бекзат Р.', score: 176, before: 108, text: 'Башында деңгээлим абдан начар болчу. B1 курсунан баштап, акырындап өстүм. Азыр 176 балл алып, медицина университетине кирдим.', emoji: '🏆' },
    { name: 'Айгерим Т.', score: 192, before: 148, text: 'Жангактагы онлайн тесттер абдан пайдалуу! Үйдө да машыгып, прогрессими байкап отурдум. Натыйжа — 192 балл. Рахмат Жангакка!', emoji: '✨' },
  ]

  const faqs = [
    { q: '📚 Кайсы предметтер курска кирет?', a: 'Математика, Аналогиялар, Текст түшүнүү, Кыргыз тили грамматикасы — ЖРТнын бардык 4 бөлүмү боюнча толук даярдык.' },
  ...
  return (
    <div style={{background:DARK, minHeight:'100vh', fontFamily:'Inter, sans-serif', color:'#fff'}}>
      ...
      {/* NAVBAR */}
      <nav className="nav-pad" style={{background:DARK, borderBottom:'1px solid rgba(255,255,255,0.08)', padding:'0 60px', height:'70px', display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, zIndex:100}}>
        ...
        <div style={{display:'flex', gap:'8px'}}>
          <button onClick={()=>setShowLogin(true)} style={{background:'rgba(255,255,255,0.08)',color:'#fff',border:'1px solid rgba(255,255,255,0.15)',borderRadius:'10px',padding:'10px 18px',fontWeight:'600',fontSize:'14px',cursor:'pointer'}}>
            Кирүү
          </button>
          <a href={whatsapp} target="_blank" rel="noopener noreferrer" style={{background:BLUE,color:'#fff',borderRadius:'10px',padding:'10px 18px',fontWeight:'800',fontSize:'14px',textDecoration:'none',display:'flex',alignItems:'center',gap:'6px'}}>
            📲 Жазылуу
          </a>
        </div>
      </nav>
      ...
      {/* HERO */}
      <div className="section-pad hero-grid" style={{padding:'80px 60px',maxWidth:'1200px',margin:'0 auto',display:'flex',alignItems:'center',gap:'60px'}}>
        <div style={{flex:'1'}}>
          ...
          <p style={{color:'rgba(255,255,255,0.6)',fontSize:'16px',lineHeight:'1.8',marginBottom:'36px',maxWidth:'480px'}}>
            Жангак 10-11-класстын окуучуларын ЖРТга инновациялык методика менен даярдайт.
          </p>
          <div style={{display:'flex',gap:'12px',flexWrap:'wrap'}}>
            <a href={whatsapp} target="_blank" rel="noopener noreferrer" style={{background:BLUE,color:'#fff',borderRadius:'14px',padding:'16px 32px',fontWeight:'900',fontSize:'16px',textDecoration:'none',display:'inline-block'}}>
              📲 Жазылуу
            </a>
            <button onClick={()=>setShowLogin(true)} style={{background:'rgba(255,255,255,0.06)',color:'#fff',border:'1px solid rgba(255,255,255,0.15)',borderRadius:'14px',padding:'16px 32px',fontWeight:'600',fontSize:'16px',cursor:'pointer'}}>
              Кирүү →
            </button>
          </div>
      ...
      {/* TIMER */}
      <div className="section-pad" style={{background:BLUE,padding:'60px',textAlign:'center'}}>
        <h2 style={{fontSize:'22px',fontWeight:'800',marginBottom:'36px',color:'rgba(255,255,255,0.9)'}}>⏰ ЖРТга чейин калды</h2>
        <div className="timer-grid" style={{display:'flex',gap:'12px',justifyContent:'center',flexWrap:'wrap',alignItems:'center'}}>
          {[
            ...
            <div key={i} style={{display:'flex',alignItems:'center',gap:'12px'}}>
              <div className="timer-box" style={{background:'rgba(255,255,255,0.15)',border:'1px solid rgba(255,255,255,0.25)',borderRadius:'16px',padding:'20px 28px',minWidth:'90px',textAlign:'center'}}>
                <div className="timer-num" style={{fontWeight:'900',fontSize:'44px',lineHeight:'1',fontVariantNumeric:'tabular-nums'}}>
                  {String(t.n).padStart(2,'0')}
                </div>
                <div style={{color:'rgba(255,255,255,0.7)',fontSize:'12px',marginTop:'6px',fontWeight:'600'}}>{t.l}</div>
              </div>
              {i<3 && <span style={{fontSize:'28px',fontWeight:'900',opacity:0.5}}>:</span>}
            </div>
          ))}
        </div>
        <a href={whatsapp} target="_blank" rel="noopener noreferrer" style={{display:'inline-block',marginTop:'32px',background:'#fff',color:BLUE,borderRadius:'12px',padding:'14px 36px',fontWeight:'900',fontSize:'15px',textDecoration:'none'}}>
          Азыр жазылуу →
        </a>
      </div>
      ...
      <div className="section-pad" style={{padding:'80px 60px',background:'#fff'}}>
        <div style={{maxWidth:'1200px',margin:'0 auto'}}>
          <div style={{textAlign:'center',marginBottom:'48px'}}>
            <h2 style={{fontSize:'32px',fontWeight:'900',color:DARK,letterSpacing:'-1px',marginBottom:'12px'}}>
          ...
          <div className="pains-grid" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'16px'}}>
            {pains.map((p,i)=>(
              <div key={i} style={{background:'#F8FAFF',border:'1px solid #E8ECF4',borderRadius:'16px',padding:'24px'}}>
          ...
          <div style={{textAlign:'center',marginTop:'36px'}}>
            <a href={whatsapp} target="_blank" rel="noopener noreferrer" style={{background:BLUE,color:'#fff',borderRadius:'14px',padding:'16px 40px',fontWeight:'900',fontSize:'16px',textDecoration:'none',display:'inline-block'}}>
              📲 Биз менен чечебиз →
            </a>
          </div>
        </div>
            ...
            {results.map((r,i)=>(
              <div key={i} style={{position:'relative',borderRadius:'20px',overflow:'hidden',aspectRatio:'3/4'}}>
                ...
                <div style={{position:'absolute',bottom:0,left:0,right:0,background:'linear-gradient(to top,rgba(13,30,74,1) 0%,rgba(13,30,74,0.4) 60%,transparent 100%)',padding:'20px 14px 14px'}}>
                  <div style={{color:'#60A5FA',fontWeight:'900',fontSize:'22px'}}>{r.score} балл</div>
      ...
      <div className="section-pad" style={{padding:'80px 60px',background:'#fff'}}>
        <div style={{maxWidth:'1200px',margin:'0 auto'}}>
          ...
          <div className="reviews-grid" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'20px'}}>
            {reviews.map((r,i)=>(
              <div key={i} style={{background:'#F8FAFF',border:'1px solid #E8ECF4',borderRadius:'20px',padding:'24px',display:'flex',flexDirection:'column',gap:'16px'}}>
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                  <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
                    <div style={{width:'44px',height:'44px',background:BLUE,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontWeight:'900',fontSize:'16px'}}>
                      {r.name[0]}
                    </div>
                    <div>
                ...
                <div style={{color:'#475569',fontSize:'13px',lineHeight:'1.7',fontStyle:'italic'}}>
                  "{r.text}"
                </div>
        ...
        <div style={{maxWidth:'1200px',margin:'0 auto'}}>
          ...
          <div className="subjects-grid" style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(170px,1fr))',gap:'16px'}}>
            {[
              ...
              <div key={s.name} style={{background:'#F8FAFF',border:'1px solid #E8ECF4',borderRadius:'16px',padding:'20px',textAlign:'center'}}>
      ...
      {/* FAQ */}
      <div id="faq" className="section-pad" style={{padding:'80px 60px',background:DARK}}>
        <div style={{maxWidth:'900px',margin:'0 auto'}}>
          <div className="faq-layout" style={{display:'flex',gap:'60px',alignItems:'flex-start'}}>
            <div style={{flex:'0 0 260px'}}>
              <h2 style={{fontSize:'32px',fontWeight:'900',lineHeight:'1.15',letterSpacing:'-1px'}}>
                Көп<br/>берилүүчү<br/>суроолор
              </h2>
              <p style={{color:'rgba(255,255,255,0.5)',fontSize:'14px',marginTop:'14px',lineHeight:'1.7'}}>
                Дагы суроолор барбы?
              </p>
              ...
              {faqs.map((faq,i)=>(
                <div key={i} style={{borderBottom:'1px solid rgba(255,255,255,0.07)'}}>
                  <button onClick={()=>setOpenFaq(openFaq===i?null:i)}
                    style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'18px 0',background:'none',border:'none',color:'#fff',cursor:'pointer',textAlign:'left',gap:'16px'}}>
                    <span style={{fontWeight:'600',fontSize:'14px'}}>{faq.q}</span>
                    <span style={{fontSize:'20px',flexShrink:0,color:'#60A5FA',transform:openFaq===i?'rotate(45deg)':'rotate(0)',transition:'transform 0.2s',display:'block'}}>+</span>
                  </button>
                  {openFaq===i && (
                    <div style={{paddingBottom:'18px',color:'rgba(255,255,255,0.6)',fontSize:'14px',lineHeight:'1.7'}}>
                      {faq.a}
                    </div>
                  )}
                </div>
      ...
      <div className="section-pad" style={{padding:'80px 60px',background:BLUE}}>
        <div style={{maxWidth:'600px',margin:'0 auto',textAlign:'center'}}>
          ...
          <p style={{color:'rgba(255,255,255,0.75)',marginBottom:'32px',fontSize:'16px'}}>
            Биз менен ЖРТга даяр болуңуз
          </p>
          <div style={{display:'flex',gap:'12px',justifyContent:'center',flexWrap:'wrap'}}>
            <a href={whatsapp} target="_blank" rel="noopener noreferrer" style={{background:'#fff',color:BLUE,borderRadius:'14px',padding:'16px 36px',fontWeight:'900',fontSize:'16px',textDecoration:'none',display:'inline-block'}}>
              📲 Жазылуу
            </a>
            <button onClick={()=>setShowLogin(true)} style={{background:'rgba(255,255,255,0.15)',color:'#fff',border:'1px solid rgba(255,255,255,0.3)',borderRadius:'14px',padding:'16px 36px',fontWeight:'700',fontSize:'16px',cursor:'pointer'}}>
              Кирүү →
            </button>
          </div>
        </div>
