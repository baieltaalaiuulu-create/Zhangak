...
export function loadConfig() {
  ...
  return Object.freeze({
    ...
    aiFallbackProvider: (process.env.AI_FALLBACK_PROVIDER ?? '').trim().toLowerCase(),
    ...
    deepseekMaxTokens: integer('DEEPSEEK_FAST_MAX_TOKENS', 900, 64, 1_200),
    openaiApiKey: process.env.OPENAI_API_KEY?.trim() || null,
    openaiBaseUrl: (process.env.OPENAI_BASE_URL ?? 'https://api.openai.com/v1').replace(/\/$/, ''),
    openaiFastModel: process.env.OPENAI_FAST_MODEL?.trim() || 'gpt-5-mini',
    openaiMaxTokens: integer('OPENAI_FAST_MAX_TOKENS', 700, 64, 1_200),
    vapidSubject,
    vapidPublicKey,
    vapidPrivateKey,
