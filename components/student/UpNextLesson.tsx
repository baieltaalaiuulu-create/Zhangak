...
import { ArrowRight, BookMarked, BookOpen, Calculator, Clock3 } from 'lucide-react'

interface Props {
  lesson: { id: string; title: string; subject: string; order_number: number; durationMinutes?: number | null } | null
  progress: number // % of series completed
}

const SUBJECT_META = {
  math: { label: 'Математика', color: 'text-blue-600', bg: 'bg-blue-50', Icon: Calculator },
  kyr: { label: 'Кыргыз тили', color: 'text-orange-600', bg: 'bg-orange-50', Icon: BookMarked },
  other: { label: 'Другие предметы', color: 'text-violet-600', bg: 'bg-violet-50', Icon: BookOpen },
}
...
export default function UpNextLesson({ lesson, progress }: Props) {
  if (!lesson) {
    return (
      <div className="rounded-2xl bg-white p-5 shadow-sm border border-gray-100 flex flex-col items-center justify-center gap-3 h-full min-h-[220px]">
        <span className="flex h-12 w-12 items-center justify-center rounded-full bg-green-50 text-green-600"><BookOpen size={24} aria-hidden="true" /></span>
  ...
  const meta = SUBJECT_META[lesson.subject as keyof typeof SUBJECT_META] ?? SUBJECT_META.other
  ...
  return (
    <div className="rounded-2xl bg-white shadow-sm border border-gray-100 overflow-hidden flex flex-col">
      ...
      <div className="p-5 flex flex-col flex-1 gap-4">
        ...
        >
          <span className="inline-flex items-center gap-1.5">Начать урок <ArrowRight size={16} aria-hidden="true" /></span>
        </Link>
      </div>
    </div>
  )
