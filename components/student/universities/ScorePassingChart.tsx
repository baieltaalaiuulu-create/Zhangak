...
export default function ScorePassingChart({ specialties, studentScore }: Props) {
  const data = specialties
    ...
    .map(s => ({ name: s.name, minScore: s.minScore as number, passes: studentScore != null && studentScore >= (s.minScore as number) }))
  const chartHeight = Math.max(220, data.length * 56)

  if (data.length === 0) {
    return (
      <div className="rounded-2xl border border-gray-100 bg-white p-8 text-center shadow-sm">
        <h3 className="text-sm font-bold text-gray-900">Проходные баллы пока не опубликованы</h3>
        <p className="mt-1 text-sm text-gray-500">Проверь актуальные требования на официальном сайте университета.</p>
      </div>
    )
  }
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
      ...
      <p className="mt-1 text-xs text-gray-400">
        {studentScore == null ? 'Пройди пробный ОРТ, чтобы увидеть свой результат на графике.' : `Красная линия — твой последний результат (${studentScore}).`}
      </p>
      ...
      <div style={{ width: '100%', height: chartHeight }} className="mt-4">
        <ResponsiveContainer>
          <BarChart data={data} layout="vertical" margin={{ top: 4, right: 24, bottom: 4, left: 4 }}>
            ...
            {studentScore != null && <ReferenceLine x={studentScore} stroke="#EF4444" strokeWidth={2} strokeDasharray="4 4" label={{ value: 'Твой балл', position: 'top', fill: '#EF4444', fontSize: 11 }} />}
