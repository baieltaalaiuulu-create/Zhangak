...
function buildAdvice(studentScore: number | null, universities: University[]): string {
  ...
  if (studentScore == null) return 'Пройди пробный ОРТ — после этого мы сопоставим твой фактический результат с опубликованными проходными баллами.'
  ...
  if (highCount === known.length) return 'Отличный результат! Ты уверенно проходишь по баллам во все вузы из подборки — можно сосредоточиться на выборе специальности и подготовке документов.'
...
export default function ScoreRecommendationBar({ studentScore, universities }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-[#6C3DE0]/15 p-5 shadow-sm" style={{ background: 'linear-gradient(135deg, #EEF2FF 0%, #F5F3FF 100%)' }}>
      <div className="flex items-center gap-2 text-sm font-bold text-[#4338CA]"><Compass size={18} aria-hidden="true" />Подбор по твоему баллу</div>
      <p className="mt-2 text-sm font-medium text-gray-600">{studentScore == null ? 'Сейчас показываем варианты по рейтингу каталога.' : `Последний пробный ОРТ: ${studentScore} баллов.`}</p>
      <div className="mt-3 flex flex-wrap gap-2">
        {universities.map(u => {
          const p = getAdmissionProbability(studentScore, u.minScore)
          return <Link key={u.id} href={`/student/online/universities/${u.id}`} className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold transition-opacity hover:opacity-80 ${PROBABILITY_STYLE[p.level]}`}>{u.shortName} — {p.label}</Link>
        })}
      </div>
      <p className="mt-3 text-sm text-gray-600">{advice}</p>
      <p className="mt-2 text-xs text-gray-500">Оценка учитывает только последний пробный балл. Требования, сроки и документы уточняй на официальном сайте вуза.</p>
