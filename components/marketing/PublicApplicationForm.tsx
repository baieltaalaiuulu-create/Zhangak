...
export default function PublicApplicationForm() {
  ...
  return (
    <section id="apply" className="w-full min-w-0 rounded-3xl bg-white p-5 text-[#0D1E4A] shadow-2xl sm:p-7">
      ...
      <form className="mt-5 grid min-w-0 gap-3" onSubmit={submit}>
        <label className="grid min-w-0 gap-1 text-sm font-bold">Имя и фамилия<input required maxLength={200} value={name} onChange={event => setName(event.target.value)} className="min-h-11 w-full min-w-0 rounded-xl border border-slate-200 px-3 text-base font-medium outline-none focus:border-[#1B3F92]" /></label>
        <label className="grid min-w-0 gap-1 text-sm font-bold">WhatsApp<input required inputMode="tel" maxLength={32} value={phone} onChange={event => setPhone(event.target.value)} placeholder="+996 555 123 456" className="min-h-11 w-full min-w-0 rounded-xl border border-slate-200 px-3 text-base font-medium outline-none focus:border-[#1B3F92]" /></label>
        <label className="grid min-w-0 gap-1 text-sm font-bold">Город<input required maxLength={120} value={city} onChange={event => setCity(event.target.value)} className="min-h-11 w-full min-w-0 rounded-xl border border-slate-200 px-3 text-base font-medium outline-none focus:border-[#1B3F92]" /></label>
        <label className="grid min-w-0 gap-1 text-sm font-bold">Курс<select required disabled={loadingCourses || courses.length === 0} value={courseId} onChange={event => setCourseId(event.target.value)} className="min-h-11 w-full min-w-0 rounded-xl border border-slate-200 bg-white px-3 text-base font-medium outline-none focus:border-[#1B3F92]"><option value="">{loadingCourses ? 'Загружаем…' : 'Выберите курс'}</option>{courses.map(course => <option key={course.id} value={course.id}>{course.name} · {course.deliveryMode === 'online' ? 'онлайн' : 'оффлайн'}</option>)}</select></label>
        {error && <p role="alert" className="rounded-xl bg-red-50 px-3 py-2 text-sm font-semibold text-red-700">{error}</p>}
        <button type="submit" disabled={submitting || loadingCourses || courses.length === 0} className="mt-1 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#E41228] px-5 text-sm font-extrabold text-white disabled:cursor-not-allowed disabled:opacity-60"><>{submitting ? <LoaderCircle className="animate-spin" size={18} aria-hidden="true" /> : <Send size={17} aria-hidden="true" />}Отправить заявку</></button>
      </form>
