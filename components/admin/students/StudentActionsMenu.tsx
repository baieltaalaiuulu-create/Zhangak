...
export default function StudentActionsMenu({ blocked, accountName, disabled = false, onResetPassword, onToggleBlock, onDelete, onChangeRole }: Props) {
  ...
  return (
    <div className="relative">
      <button type="button" onClick={() => setOpen(v => !v)} aria-label={accountName ? `Действия для ${accountName}` : 'Действия'} disabled={disabled}
        className="flex h-11 w-11 items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50">
            ...
            {items.map(item => {
              ...
              return (
                <button key={item.label} type="button" onClick={() => { setOpen(false); item.onClick() }} disabled={disabled}
                  className={`flex min-h-11 w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-sm font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${item.danger ? 'text-red-500 hover:bg-red-50' : 'text-gray-600 hover:bg-gray-50'}`}>
