        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select id="account-role" value={role} onChange={event => setRole(event.target.value as AccountRole)} disabled={allowedRoles.length <= 1}
              className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[#1B4FD8]/20 disabled:cursor-not-allowed disabled:bg-gray-50">
                ...
                <div className="flex flex-wrap gap-2">
                  {STUDENT_TYPES.map(option => (
                    <button key={option.value} type="button" onClick={() => setStudentType(option.value)}
                      className={`rounded-lg border px-3 py-1.5 text-sm font-semibold transition-colors ${studentType === option.value ? 'border-[#1B4FD8] bg-[#EEF2FF] text-[#1B4FD8]' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
                      {option.label}
                    </button>
                  ))}
                </div>
