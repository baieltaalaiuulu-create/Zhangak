...
import { updateAdminCourse, type AdminCourse } from '@/lib/admin-learning-client'
...
export default function CourseEditModal({ course, onClose, onSaved }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : 'Не удалось сохранить курс')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <section className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl" onClick={event => event.stopPropagation()} aria-labelledby="course-edit-title">
        <div className="mb-5 flex items-center justify-between gap-4">
          <div>
            <h2 id="course-edit-title" className="text-lg font-bold text-[#191B23]">Настроить курс</h2>
            <p className="mt-1 text-sm text-gray-500">Архивный курс остаётся в данных, но недоступен ученикам.</p>
          </div>
